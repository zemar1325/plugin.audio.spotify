...
class ConnectDaemon(threading.Thread):
    ...
    def run(self):
        ...
        if xbmcvfs.exists("/run/libreelec/"):
            # avahi on libreelec conflicts with the mdns implementation of librespot
        ...
        try:
            try:
                ...
                try:
                    log_exception("trying AP Port 80", xbmc.LOGINFO)
                    self.__spotty_proc \
                        = self.__spotty.run_spotty(arguments=spotty_args,
                                                   disable_discovery=disable_discovery,
                                                   ap_port="80")
                except:
                    log_exception("trying AP Port 4070", xbmc.LOGINFO)
                    self.__spotty_proc \
                        = self.__spotty.run_spotty(arguments=spotty_args,
                                                   disable_discovery=disable_discovery,
                                                   ap_port="4070")
            while not self.__exit:
                ...
                if self.__spotty_proc.returncode \
                        and self.__spotty_proc.returncode > 0 and not self.__exit:
            ...
            self.daemon_active = False
            log_exception("Cannot run SPOTTY, No APs available", xbmc.LOGINFO)
