...
class Spotify(object):
    ...
    def artist_albums(self, artist_id, album_type=None, country=None, limit=20,
        ...
        return self._get('artists/' + trid + '/albums', album_type=album_type,
                         country=country, limit=limit, offset=offset, market=market)
    ...
    def search(self, q, limit=10, offset=0, type_str='track', market=None):
        """ searches for an item
            ...
            Parameters:
                ...
                - limit  - the number of items to return
                - offset - the index of the first item to return
                - type_str - the type of item to return. One of 'artist', 'album',
                         'track' or 'playlist'
                - market - An ISO 3166-1 alpha-2 country code or the string from_token.
        """
        return self._get('search', q=q, limit=limit, offset=offset, type=type_str, market=market)
    ...
    def user_playlist(self, user, playlist_id=None, market=None, fields=None):
        ...
        return self._get("users/%s/playlists/%s" % (user, plid), fields=fields, market=market)
    ...
    def user_playlist_is_following(self, playlist_owner_id, playlist_id, user_ids):
        ...
        """
        return self._get(
                "users/{}/playlists/{}/followers/contains?ids={}".format(playlist_owner_id,
                                                                         playlist_id,
                                                                         ','.join(user_ids)))
    ...
    def current_user_saved_tracks(self, market=None, limit=20, offset=0):
        ...
        """ Get details for a single category
            ...
            Parameters:
                ...
                - locale - The desired language, consisting of an ISO 639
                  ...
                  by an underscore.
        """
        return self._get('browse/categories/%s' % category_id, country=country, locale=locale)
    ...
    def following_contains(self, type_str, ids):
        """ Checks if the user follows an artist or user

            Parameters:
                - type_str - artist/user
                - ids      - list of spotify id's

        """
        return self._get('me/following/contains?type=%s&ids=%s' % (type_str, ids))
    ...
    def follow(self, type_str, ids):
        return self._put('me/following?type=%s&ids=%s' % (type_str, ids))

    def unfollow(self, type_str, ids):
        return self._delete('me/following?type=%s&ids=%s' % (type_str, ids))
    ...
    def _get_id(self, type_str, id_str):
        fields = id_str.split(':')
        if len(fields) >= 3:
            if type_str != fields[-2]:
                self._warn('expected id of type %s but found type %s %s',
                           type_str, fields[-2], id_str)
            return fields[-1]
        fields = id_str.split('/')
        if len(fields) >= 3:
            itype = fields[-2]
            if type_str != itype:
                self._warn('expected id of type %s but found type %s %s',
                           type_str, itype, id_str)
            return fields[-1]
        return id_str

    def _get_uri(self, type_str, id_str):
        return 'spotify:' + type_str + ":" + self._get_id(type_str, id_str)
